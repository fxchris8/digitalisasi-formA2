...
export async function findByUserName(userName: string): Promise<User | null> {
	const result = await pool.query<User>("SELECT * FROM users WHERE user_name = $1 LIMIT 1", [
		userName,
	])
...
export async function createUser(
	...
	if (!row) throw new AppError("Unexpected database error", 500, "INTERNAL_SERVER_ERROR")
	return row
}
